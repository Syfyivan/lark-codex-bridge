...
托盘菜单「大小」可选 很小 / 小 / 中 / 大（持久化到 `userData/kodama-window.json`，桌宠按窗口自适应缩放）。窗口用 `screen-saver` 层级 + `visibleOnFullScreen` 并在 show 时重申，尽量浮在其它 App 全屏之上；macOS 上若个别全屏仍压不住，后续可接 `electron-panel-window`（NSPanel）彻底解决。
