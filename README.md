...
- **菜单栏 Kodama** → 显示/隐藏、事件 / 配置面板、勿扰、开机自启、番茄钟、大小、配饰、退出；找不到菜单栏入口时可按 `⌘⌥K` 显示/隐藏、`⌘⌥P` 打开面板，或在本仓库运行 `pnpm run show`
...
托盘菜单「大小」可选 很小 / 小 / 中 / 大（持久化到 `userData/kodama-window.json`，桌宠按窗口自适应缩放）。右键面板还能实时调「图案大小」「透明度」「点击范围」「触发方式」「气泡位置」「面板位置」「气泡高度」「气泡间距」，设置存在 Electron 本地存储里。

默认窗口是 `280x400`，图案默认 `72%`、透明度 `82%`、命中框 `35%`，气泡/面板默认贴近宠物但不压住宠物。旧版本地配置会迁移一次，避免继续沿用过大的窗口和透明边界命中。
...
也可以直接访问本地控制接口：`http://127.0.0.1:7766/pet/show`、`/pet/panel`、`/pet/hide`、`/pet/toggle`。
